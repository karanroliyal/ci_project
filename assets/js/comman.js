...
$("#imageId").on('input', function () {
    ...
    if ($("#imageId").val() == "") {
        ...
        document.getElementById("myUploadView").src = window.URL.createObjectURL(this.files[0]);
    }
})
...
function resetLoadTable() {
    setInterval(function () { getTable() }, 50);
}
        ...
        success: function (data) {
            ...
            Object.keys(data).forEach(function (key) {
                ...
                if (inputField.attr('type') !== 'file' && inputField.attr('type') !== 'password') {
                    $(`#tableData  input[name='${key}']`).val(data[key]);
                }
            })
            ...
            if (data.image) {
                ...
                $("#myUploadView").attr('src', baseUrls + imagePath.slice(2) + "/" + imageName)
            }
